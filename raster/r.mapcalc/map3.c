                      ...
                      int row)
{
    ...
    case CELL_TYPE:
        for (i = 0; i < columns; i++) {
            ...
            if (Rast3d_put_value(handle, i, row, depth, (char *)&x,
                                 DCELL_TYPE) < 0)
...
static void translate_from_colors(map *m, DCELL *rast, CELL *cell, int ncols,
                                  int mod)
{
    int i;
...
static void close_map(map *m)
{
    ...
    if (!Rast3d_close(m->handle))
        G_fatal_error(_("Unable to close raster map <%s@%s>"), m->name,
                      m->mapset);
    ...
    case 'M':
        ...
        if (mapset) {
            ...
            handle = Rast3d_open_cell_old(tmpname, mapset, &current_region3,
                                          ...
                                          RASTER3D_NO_CACHE);
            result = (Rast3d_file_type_map(handle) == FCELL_TYPE) ? FCELL_TYPE
                                                                  : DCELL_TYPE;
            Rast3d_close(handle);
        }
        else
...
int open_map(const char *name, int mod, int row, int col)
{
    ...
    m->handle =
        Rast3d_open_cell_old((char *)name, (char *)mapset, &current_region3,
                             DCELL_TYPE, RASTER3D_USE_CACHE_DEFAULT);
...
int open_output_map(const char *name, int res_type)
{
    ...
    handle = Rast3d_open_new_opt_tile_size(
        (char *)name, RASTER3D_USE_CACHE_XYZ, &current_region3,
        res_type == FCELL_TYPE ? FCELL_TYPE : DCELL_TYPE, 32);
