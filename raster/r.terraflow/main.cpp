...
void parse_args(int argc, char *argv[])
{
    ...
    output_elev->description =
        _("Name for output filled (flooded) elevation raster map");
    ...
    output_watershed->description =
        _("Name for output sink-watershed raster map");
    ...
    output_accu->description =
        _("Name for output flow accumulation raster map");
    ...
    stats_opt->description =
        _("Name for output file containing runtime statistics");
...
void check_header(char *cellname)
{
    ...
    if (!((region->ew_res == cell_hd.ew_res) &&
        ...
        G_fatal_error(
            _("Raster map <%s> resolution differs from current region"),
            cellname);
    }
    else {
        G_verbose_message(
            _("Header of raster map <%s> compatible with region header"),
            cellname);
    }
...
/* ---------------------------------------------------------------------- */
void setFlowAccuColorTable(char *cellname)
{
...
/* ---------------------------------------------------------------------- */
void setSinkWatershedColorTable(char *cellname)
{
    struct Colors colors;
    const char *mapset;
    struct Range r;
...
/* print the largest interim file that will be generated during
   r.terraflow */
void printMaxSortSize(long nodata_count)
{
    char buf[BUFSIZ];
    ...
    G_debug(1, "available %ld blocks x %ldB = %.0fB", (long)statbuf.f_bavail,
            statbuf.f_bsize, avail);
    if (avail > maxneed) {
        G_debug(1, ". OK.");
    }
    else {
...
int main(int argc, char *argv[])
{
    ...
    if ((nr > dimension_type_max) || (nc > dimension_type_max)) {
        G_fatal_error(_("[nrows=%d, ncols=%d] dimension_type overflow -- "
                        "change dimension_type and recompile"),
                      nr, nc);
    }
    else {
    ...
    if (opt->stats) {
        ...
        record_args(argc, argv);
        {
            ...
            *stats << "region size = " << formatNumber(buf, grid_size)
                   << " elts "
    ...
    nodataType::init();
    if (stats)
        *stats << "internal nodata value: " << nodataType::ELEVATION_NODATA
               << endl;
  ...
  G_verbose_message(_("jumping to flow accumulation computation\n");
    ...
    if (opt->flowaccu_grid && opt->tci_grid)
        ...
        stream2_FCELL(outstr, nrows, ncols, printTci(), opt->tci_grid);
